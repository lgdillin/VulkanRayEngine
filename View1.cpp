...
void View::drawDot() {
	m_dot.bind(getCurrentFrame().commandBuffer);
	m_dot.draw(getCurrentFrame().commandBuffer);
}
...
void View::initialize() {
	...
	m_dot = Dot(m_device, m_gpu, vertices, 400, 300);
	m_dot.createPipeline();
}
...
void View::initDot() {
	// we can interleave all of our vertex data into one single array with one binding,
	// or we can have multiple arrays that store each part of the vertex data separately,
	// with different bindings per array.
	// however, in order to make this work, we need to describe how the data is laid out
	// so we need to provide a vertex binding description at pipeline creation

	// in the description, we need to identify
	// binding index (attr number)
	// input rate (ie VK_VERETX_INPUT_RATE_VERTEX) <- most common used, 
	//		alt is VK_VERTEX_INPUT_RATE_INSTANCE, when working with instance'd data
	// stride: interval in bytes from one data entry to the next

	
	// for each attribute, we must provide a vertex attr desc
	// 4 pieces of info
	// binding index for the vertex buffer the attribute is located in
	// location: locatino value for the input in the vertex shader
	// offset: offset of the attribute in byte from the vertex start
	// vkformat: vkformat value describing the type of data 
	// example:
	// layout(location =1) in vec3 color;
	// binding = 0 // assuming data is interleaved in the same array
	// location = 1 // second attr 
	// offset = 8 // if the position data is vec2, then 4 bytes * 2 = 8
